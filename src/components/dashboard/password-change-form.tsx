...
export function PasswordChangeForm({
  ...
  async function submitPasswordChange(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (payload?.success !== true) {
        throw new Error("The server could not confirm the password change.");
      }
  ...
  return (
    ...
    >
      ...
      />
      <PasswordFeedback error={error} logoutError={displayedLogoutError} />
      <PasswordFormActions
        mode={mode}
        onLogout={onLogout}
        signOut={signOut}
...
function PasswordFields({
  ...
  isBusy: boolean;
  clearServerFieldError: (field: PasswordField) => void;
}) {
  return (
    <FieldGroup>
      {requireCurrentPassword && (
      ...
      <Field data-invalid={!!fieldErrors.newPassword}>
        ...
        <PasswordInput
          ...
          aria-describedby={`${newPasswordId}-description${fieldErrors.newPassword ? ` ${newPasswordId}-error` : ""}`}
          required
          disabled={isBusy}
        />
...
function PasswordFeedback({
  ...
  return (
    <>
      {error && <FieldError id="password-change-error">{error}</FieldError>}
      {logoutError && <FieldError>{logoutError}</FieldError>}
    </>
  );
}
  ...
  if (mode === "dialog") {
    return (
      <DialogFooter className="mt-1 sm:justify-between">
        {onLogout && (
          <Button type="button" variant="outline" className="w-full sm:w-auto" disabled={isBusy} onClick={() => void signOut()}>
            <ActionSpinner busy={isSigningOut} />
            {isSigningOut ? "Signing out..." : "Sign out"}
          </Button>
        )}
        <Button type="submit" className="w-full sm:w-auto" disabled={isBusy}>
          <ActionSpinner busy={isSubmitting} />
